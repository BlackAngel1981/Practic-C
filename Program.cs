...
void CheckingNum(string number)
{
  ...
  {
    Console.WriteLine($"Your number: {number} - палиндром.");
  }
  else Console.WriteLine($"Your number: {number} - НЕ палиндром.");
}
...
{
    Console.Write($"Input coordinate {coordName} point {pointName}: ");
